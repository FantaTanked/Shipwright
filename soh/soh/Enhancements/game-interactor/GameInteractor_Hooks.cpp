...
bool GameInteractor_Should(GIVanillaBehavior flag, u32 result, ...) {
    // Capture the variadic args into a va_list to forward to the hook subscribers. Each subscriber copies the
    // list before reading so it doesn't advance the shared pointer for the others.
    va_list args;
    va_start(args, result);

    // `result` is declared as int (not bool) because default argument promotion requires that for va_start;
    // downcast it back to bool here for the hook handlers.
...
void GameInteractor_ExecuteOnZeldaArenaCleanup() {
