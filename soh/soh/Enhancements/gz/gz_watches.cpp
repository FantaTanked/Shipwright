// gz-style watches for ship -- see gz_watches.h for the design and threading notes.
...
#define CVAR_GZ_WATCHES_NAME CVAR_CHEAT("GzWatches")
...
struct GzWatchCatalog {
    const char* label;
    GzWatchSource source;
    GzWatchType naturalType;
};

static const GzWatchCatalog kCatalog[] = {
    { "pos x", SRC_POS_X, GZ_WATCH_F32 },
    { "pos y", SRC_POS_Y, GZ_WATCH_F32 },
    { "pos z", SRC_POS_Z, GZ_WATCH_F32 },
    { "speed", SRC_SPEED, GZ_WATCH_F32 },
    { "y vel", SRC_YVEL, GZ_WATCH_F32 },
    { "facing", SRC_FACING, GZ_WATCH_S16 },
    { "cam yaw", SRC_CAM_YAW, GZ_WATCH_S16 },
    { "cam pitch", SRC_CAM_PITCH, GZ_WATCH_S16 },
    { "frames", SRC_FRAMES, GZ_WATCH_U32 },
};
...
static const char* const kTypeName[GZ_WATCH_TYPE_COUNT] = { "u8",  "s8",  "x8",  "u16", "s16",
                                                            "x16", "u32", "s32", "x32", "f32" };
static const int kTypeSize[GZ_WATCH_TYPE_COUNT] = { 1, 1, 1, 2, 2, 2, 4, 4, 4, 4 };

static void FormatValue(GzWatchType type, uint64_t bits, char* out, size_t n) {
    switch (type) {
        case GZ_WATCH_U8:
...
static bool ReadSource(GzWatchSource src, PlayState* play, uint64_t* bits) {
...
static std::vector<GzWatch> sWatches;

static std::mutex sSnapshotMutex;
static std::vector<GzWatchDisplay> sSnapshot;
...
static void GzWatch_Save() {
    ...
    CVarSetString(CVAR_GZ_WATCHES_NAME, s.c_str());
...
bool GzWatch_Add(int catalogIndex) {
...
// in bounds (the one safe deviation from gz's any-type-on-any-address).
void GzWatch_CycleType(int i, int dir) {
    if (i < 0 || i >= (int)sWatches.size()) {
        return;
    }
    const int size = kTypeSize[sWatches[i].type];
    int t = sWatches[i].type;
    for (int step = 0; step < GZ_WATCH_TYPE_COUNT; step++) {
        t = (t + (dir >= 0 ? 1 : GZ_WATCH_TYPE_COUNT - 1)) % GZ_WATCH_TYPE_COUNT;
        if (kTypeSize[t] == size) {
            sWatches[i].type = (GzWatchType)t;
            break;
        }
    }
    GzWatch_Save();
...
void GzWatch_Load() {
    sWatches.clear();
    std::string s = CVarGetString(CVAR_GZ_WATCHES_NAME, "");
    size_t pos = 0;
    while (pos < s.size() && (int)sWatches.size() < kMaxWatches) {
        ...
        if (sscanf(s.c_str() + pos, "%d,%d,%d,%d", &cat, &type, &x, &y) == 4 && cat >= 0 &&
            cat < kCatalogCount && type >= 0 && type < GZ_WATCH_TYPE_COUNT) {
            GzWatch w;
            w.catalogIndex = cat;
            w.type = (GzWatchType)type;
            w.x = (float)x;
            w.y = (float)y;
            sWatches.push_back(w);
        }
...
void GzWatch_UpdateSnapshot(void* playState) {
    ...
    std::vector<GzWatchDisplay> snap;
