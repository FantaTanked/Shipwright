...
// One declarative field table (savestate_static_fields.inc, generated by tools/gen_savestate_fields.py from
// the capture side) drives BOTH capture and restore for the bulk of the snapshotted decomp statics, so the two
// directions can no longer drift -- several fields used to be captured but never restored. SS_SCALAR = a plain
// assignable; SS_BLOCK = an array/struct copied by the snapshot field's size (the live globals are sometimes
// incomplete-array externs whose size isn't visible here, so the bound always comes from the snapshot member).
