...
// ship-gz: read-only accessors over the curated scene list, so features like the gz
...
const char* SceneSelect_GetSceneName(s32 index) {
    ...
    // Skip the leading " N:" numbering for a cleaner gz-style label.
...
// Per-entrance accessors, so the gz warps menu can offer every named entrance of a scene
