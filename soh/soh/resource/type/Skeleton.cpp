...
std::vector<std::pair<void*, size_t>> Skeleton::GetSubAllocations() {
    ...
    switch (type) {
        case SkeletonType::Normal:
            segment = skeletonData.skeletonHeader.segment;
            count = skeletonData.skeletonHeader.limbCount;
            break;
        case SkeletonType::Flex:
            segment = skeletonData.flexSkeletonHeader.sh.segment;
            count = skeletonData.flexSkeletonHeader.sh.limbCount;
            break;
        case SkeletonType::Curve:
            segment = (void**)skeletonData.skelCurveLimbList.limbs;
            count = skeletonData.skelCurveLimbList.limbCount;
            break;
        default:
            break;
    }
    if (segment == nullptr || count == 0) {
        return {};
    }
    return { { (void*)segment, count * sizeof(void*) } };
}
