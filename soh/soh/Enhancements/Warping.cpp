...
// ship-gz: curated scene list accessors (see z_select.c).
...
// uses a saved respawn position). Used by the gz default-warps list.
...
// ---------------------------------------------------------------------------------------
// gz-style hierarchical warp browser (category -> place -> entrance).
//
...
struct GzCategory {
    ...
    int sceneCount;
    const GzBoss* bosses;  // flat: the boss targets
    int bossCount;
};

// Non-flat category membership (scene indices). See the §3 table in GZ_WARPS_HANDOVER.md.
...
#define GZ_SCENES(arr) (arr), (int)(sizeof(arr) / sizeof((arr)[0]))

const GzCategory kCategories[] = {
    { "dungeons", false, GZ_SCENES(kDungeonScenes), nullptr, 0 },
    { "bosses", true, nullptr, 0, kBosses, (int)(sizeof(kBosses) / sizeof(kBosses[0])) },
    { "towns", false, GZ_SCENES(kTownScenes), nullptr, 0 },
    { "houses", false, GZ_SCENES(kHouseScenes), nullptr, 0 },
    { "shops", false, GZ_SCENES(kShopScenes), nullptr, 0 },
    { "overworld", false, GZ_SCENES(kOverworldScenes), nullptr, 0 },
    { "misc", false, GZ_SCENES(kMiscScenes), nullptr, 0 },
};
...
const GzCategory* GzCat(int cat) {
...
const char* GzWarp_CategoryName(int cat) {
    const GzCategory* c = GzCat(cat);
    return c ? c->name : "";
}

bool GzWarp_CategoryIsFlat(int cat) {
    const GzCategory* c = GzCat(cat);
    return c && c->flat;
}
