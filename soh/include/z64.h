...
typedef enum { LANGUAGE_ENG, LANGUAGE_GER, LANGUAGE_FRA, LANGUAGE_JPN, LANGUAGE_MAX } Language;
...
typedef struct {
    ...
    /*        */ u8 mapPalettesPulse[40][32]; // Used to have unique pointers per map pulse color for the shader
                                              // backend. 40 for map pulse timer x2
    // #endregion
...
typedef enum {
    /* 0 */ LENS_MODE_HIDE_ACTORS, // lens actors are visible by default, and hidden by using lens (for example, fake
                                   // walls)
    /* 1 */ LENS_MODE_SHOW_ACTORS  // lens actors are invisible by default, and shown by using lens (for example,
                                   // invisible enemies)
} LensMode;
...
#define TRANS_NEXT_TYPE_DEFAULT \
    0xFF // when `nextTransitionType` is set to default, the type will be taken from the entrance table for the ending
         // transition
...
#define TRANS_TYPE_CIRCLE(appearance, color, speed) \
    ((1 << 5) | ((color & 3) << 3) | ((appearance & 3) << 1) | (speed & 1))
...
typedef enum { STACK_STATUS_OK = 0, STACK_STATUS_WARNING = 1, STACK_STATUS_OVERFLOW = 2 } StackStatus;
